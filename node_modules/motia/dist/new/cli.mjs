...
const generateIndex = () => {
	...
	const steps = [...getStepFilesFromDir(path.join(process.cwd(), "steps")), ...getStepFilesFromDir(path.join(process.cwd(), "src"))].map((file) => {
		const constName = toSnakeCaseConst(file);
		return {
			importStatement: `import * as ${constName} from '${file}';`,
...
function schemaToJsonSchema(schema) {
	...
	if (isZodSchema(schema)) return z.toJSONSchema(schema, { target: "draft-7" });
...
const generateTypes = async (output) => {
	try {
		...
		writeFileSync(output, generateTypesString(steps, streams));
	} catch (error) {
...
async function create() {
	...
	try {
		...
		for (const dir of dirs) await mkdir(join(targetDir, dir), { recursive: true });
		try {
			for (const file of files) {
