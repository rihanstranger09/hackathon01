...
- either make a new Command for each call to parse, or stop storing options as properties`);
 ...
 - ${executableDir ? `searched for local subcommand relative to directory '${executableDir}'` : "no directory for search for local subcommand, use .executableDir() to supply a custom directory"}`;
		...
		*/
		executableDir(path$4) {
			if (path$4 === void 0) return this._executableDir;
			this._executableDir = path$4;
			return this;
		}
		/**
...
const getStreamFilesFromDir = (dir) => {
	...
	return (0, glob.globSync)("**/*.stream.{ts,js}", {
		absolute: true,
		cwd: dir
	}).map(toRelativePosix);
};
...
const toSnakeCaseConst = (filePath) => {
	let relPath = path.default.relative(process.cwd(), filePath).replace(/\\/g, "/");
...
const generateIndex = () => {
	const motiaConfigPath = path.default.join(process.cwd(), "motia.config.ts");
	const hasMotiaConfig = (0, fs.existsSync)(motiaConfigPath);
	const motiaConfigContent = hasMotiaConfig ? (0, fs.readFileSync)(motiaConfigPath, "utf8") : "";
	...
	const streams = [
		...getStreamFilesFromDir(path.default.join(process.cwd(), "streams")),
		...getStreamFilesFromDir(path.default.join(process.cwd(), "src")),
		...getStreamFilesFromDir(path.default.join(process.cwd(), "steps"))
	].map((file) => {
	...
	const steps = [...getStepFilesFromDir(path.default.join(process.cwd(), "steps")), ...getStepFilesFromDir(path.default.join(process.cwd(), "src"))].map((file) => {
		const constName = toSnakeCaseConst(file);
		return {
			importStatement: `import * as ${constName} from '${file}';`,
	...
	return [
		...
		hasMotiaConfig && hasAuthenticateStream ? `motia.authenticateStream = motiaConfig.authenticateStream;` : "// No authenticateStream found in motia.config.ts",
		"motia.initialize();"
	].join("\n");
};

//#endregion
//#region src/new/build/build.ts
const build = (options) => {
...
function schemaToJsonSchema(schema) {
	...
	if (isZodSchema(schema)) return zod.toJSONSchema(schema, { target: "draft-7" });
...
//#endregion
//#region src/types/schema.types.ts
...
const generateStreamsInterface = (streams) => {
	const entries = Object.entries(streams).filter(([_, stream]) => !stream.hidden).map(([name, stream]) => {
		const jsonSchema = schemaToJsonSchema(stream.config.schema);
		return `    ${name}: MotiaStream<${jsonSchema ? generateTypeFromSchema(jsonSchema) : "unknown"}>`;
	});
	return entries.length > 0 ? `  interface Streams {\n${entries.join("\n")}\n  }` : `  interface Streams {}`;
};
const generateEnqueuesInterface = (steps) => {
	const enqueueTypes = {};
	for (const step of steps) {
		if (step.config.enqueues) step.config.enqueues.forEach((enqueue) => {
			const topic = typeof enqueue === "string" ? enqueue : enqueue.topic;
			if (!enqueueTypes[topic]) enqueueTypes[topic] = "unknown";
		});
		for (const trigger of step.config.triggers) if (trigger.type === "queue") {
			const topic = trigger.topic;
			if (!enqueueTypes[topic] && trigger.input) {
				const jsonSchema = schemaToJsonSchema(trigger.input);
				enqueueTypes[topic] = jsonSchema ? generateTypeFromSchema(jsonSchema) : "unknown";
			}
		}
	}
...
const generateTypesString = (steps, streams) => {
	return `/**
...
${generateStreamsInterface(streams)}

${generateEnqueuesInterface(steps)}
}
`;
};
...
const loadStepsAndStreams = async () => {
	const stepFiles = await (0, glob.glob)("**/*.step.{ts,js}", {
		cwd: process.cwd(),
		ignore: ["node_modules/**", "dist/**"]
	});
	const streamFiles = await (0, glob.glob)("**/*.stream.{ts,js}", {
		cwd: process.cwd(),
		ignore: ["node_modules/**", "dist/**"]
	});
	...
	return {
		...
		streams: Object.fromEntries(streamResults.filter(Boolean).map((s) => [s?.config?.name, s]))
	};
};
const loadFile = async (file, type) => {
	try {
		...
		const tempFile = path.default.join(os.default.tmpdir(), `motia-${type}-${Date.now()}-${Math.random().toString(36).slice(2)}.mjs`);
		fs.default.writeFileSync(tempFile, result.outputFiles[0].text);
		const module$1 = await import((0, url.pathToFileURL)(tempFile).href);
		fs.default.unlinkSync(tempFile);
		if (type === "step") return {
			config: module$1.config,
			filePath: file,
			handler: module$1.handler,
...
//#endregion
//#region src/new/build/typegen.ts
const typegen = async (options = {}) => {
	await generateTypes(options.output ?? "types.d.ts");
};
const generateTypes = async (output) => {
	try {
		const { steps, streams } = await loadStepsAndStreams();
		(0, fs.writeFileSync)(output, generateTypesString(steps, streams));
	} catch (error) {
...
function fetchHeaders() {
	const headers = { "User-Agent": "motia-cli" };
	const token = process.env.GITHUB_TOKEN;
	if (token) headers.Authorization = `Bearer ${token}`;
	return headers;
}
async function fetchRepoTree() {
	const url$1 = `https://api.github.com/repos/${REPO}/git/trees/${BRANCH}?recursive=1`;
	...
	if (res.status === 403 || res.status === 429) {
		...
		const resetMsg = resetHeader ? ` Rate limit resets at ${(/* @__PURE__ */ new Date(Number(resetHeader) * 1e3)).toLocaleTimeString()}.` : "";
		throw new Error(`GitHub API rate limit exceeded.${resetMsg} Set GITHUB_TOKEN to increase your limit.`);
	}
	if (!res.ok) throw new Error(`Failed to fetch template repository: ${res.statusText}`);
	return (await res.json()).tree.filter((entry) => entry.type === "blob" && !SKIP_FILES.has(entry.path));
}
async function downloadFile(filePath) {
	const url$1 = `https://raw.githubusercontent.com/${REPO}/${BRANCH}/${filePath}`;
	const res = await fetch(url$1, {
		headers: fetchHeaders(),
		signal: AbortSignal.timeout(FETCH_TIMEOUT_MS)
	});
	if (!res.ok) throw new Error(`Failed to download ${filePath}: ${res.statusText}`);
	return res.text();
}
async function create() {
	console.log(BANNER);
	const rl = (0, readline.createInterface)({
		input: process.stdin,
	...
	try {
		...
		while (true) {
			...
			emptyCount = 0;
			targetDir = (0, path.join)(process.cwd(), folderName);
			if ((0, fs.existsSync)(targetDir)) {
		...
		try {
			...
			if (name === "TimeoutError" || name === "AbortError") console.error(`\n  ${RED}Connection timed out.${R} Check your internet connection and try again.\n`);
			else if (msg.includes("rate limit")) console.error(`\n  ${RED}${msg}${R}\n`);
		...
		await (0, fs_promises.mkdir)(targetDir, { recursive: true });
		for (const dir of dirs) await (0, fs_promises.mkdir)((0, path.join)(targetDir, dir), { recursive: true });
		try {
			for (const file of files) {
				...
				await (0, fs_promises.writeFile)((0, path.join)(targetDir, file.path), content);
			}
		} catch (err) {
			const name = err instanceof Error ? err.name : "";
			const msg = err instanceof Error ? err.message : String(err);
			...
			try {
				await (0, fs_promises.rm)(targetDir, {
					recursive: true,
					force: true
				});
		...
		console.log("");
		try {
			(0, child_process.execSync)("npm install", {
				cwd: targetDir,
				stdio: "inherit"
			});
		} catch {
...
//#endregion
//#region src/new/cli.ts
const program = new Command();
program.command("dev").description("Build the project for development").action(() => {
	dev().catch((err) => {
		console.error(err);
		process.exitCode = 1;
	});
});
program.command("build").description("Build the project for production").option("-e, --external <external>", "External dependencies").action((options) => {
	build({ external: options.external ? options.external.split(",") : [] }).catch((err) => {
		console.error(err);
		process.exitCode = 1;
	});
});
program.command("typegen").description("Generate TypeScript types from steps and streams").option("-w, --watch", "Watch for file changes").option("-o, --output <path>", "Output file path", "types.d.ts").action((options) => {
	typegen(options).catch((err) => {
		console.error(err);
		process.exitCode = 1;
	});
});
program.command("create").description("Create a new Motia project powered by iii").action(() => {
	create().catch((err) => {
		console.error(err);
		process.exitCode = 1;
	});
